...
/*   Updated: 2023/03/11 01:25:24 by samini           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_prepare_cmd(int *in_cmd, t_cmds **cmds, t_seer *seer, t_token *tok)
{
	...
	if (ft_strchr(tok[1].value, '*') && !tok[1].t_in_q)
	{
		tmp = tok[1].value;
		tok[1].value = ft_strtrim(tok[1].value, "\"");
		free(tmp);
		tmp = tok[1].value;
		tok[1].value = get_wildcard_string(tok[1].value, seer);
		free(tmp);
		if (ft_strchr(tok[1].value, ' '))
			ft_split_wilds(cmds, tok, in_cmd);
	}
